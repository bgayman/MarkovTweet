...
class MarkovGenerator
{
    static func processText(text: String, lookbehind: Int = 2, splitBy: Split) -> [NSArray: [[Double: GKState]]]
    {
        ...
        for (bufferArray, countedSet) in substringsToCountedSets
        {
            ...
            {
                guard let count = countedSet.countForObject(obj) else { continue }
    ...
    private static func prefixToCountedSetOfChars(string string: String, lookbehind: Int) -> [String: NSCountedSet]
    {
        ...
        for i in lookbehind ..< string.characters.count
        {
            ...
            let prefix = string.substringWithRange(start ..< end)
            ...
            let next = string.substringWithRange(end ..< end.advancedBy(1))
...
extension NSCountedSet
{
    func uniformCountForObject(a: AnyObject) -> Double
    {
        ...
        return Double(self.countForObject(a)) / Double(total)
    }
}
...
class StringState: GKState, NSCoding
{
    ...
    {
        let name = self.classNameForString(string)
    ...
    {
        let cls: AnyClass = self.classForString(string)
...
class CountedSet<T: Hashable>
{
    ...
    var allObjects: [T]{
        return Array(self.objectToCount.keys)
    }
    ...
    {
        if let count = self.objectToCount[value]
        {
            self.objectToCount[value] = count + 1
        }
        else
        {
            self.objectToCount[value] = 1
        }
        self.totalCount += 1
    }
    ...
    {
        return self.objectToCount[value]
    }
}
